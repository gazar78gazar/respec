...
interface ConflictPanelProps {
  ...
  onResolveConflict: (conflictId: string, action: ConflictSuggestion['action'], newValue?: string) => Promise<void>;
  onDismissConflict?: (conflictId: string) => void;
  className?: string;
}

function ConflictPanel({ conflicts, onResolveConflict, onDismissConflict, className = '' }: ConflictPanelProps) {
  ...
  useEffect(() => {
    ...
    if (highSeverityConflicts.length > 0) {
      setExpandedConflicts(prev => new Set([...prev, ...highSeverityConflicts]));
    }
  }, [conflicts]);
  ...
  const getTypeLabel = (type: FieldConflict['type']) => {
    switch (type) {
      case 'value_change': return 'Value Change';
      case 'constraint_violation': return 'Constraint Violation';
      case 'incompatible_specs': return 'Compatibility Issue';
      case 'overwrite_warning': return 'Overwrite Warning';
  ...
  const severityOrder: FieldConflict['severity'][] = ['critical', 'error', 'warning', 'info'];
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex space-x-2 text-xs">
            {Object.entries(groupedConflicts).map(([severity, conflictList]) => (
              <span key={severity} className={`px-2 py-1 rounded-full ${getSeverityColor(severity)}`}>
            ...
            <div key={severity}>
              {severityConflicts.map(conflict => {
                ...
                return (
                  <div
                    key={conflict.id}
                    className={`border-b border-l-4 ${getSeverityColor(conflict.severity)} last:border-b-0`}
                  >
                    <div
                    ...
                    >
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-gray-600">
                              {getTypeLabel(conflict.type)} • {Math.round(conflict.confidence * 100)}% confidence
                            </div>
                          </div>
                        </div>
                      ...
                      {!isExpanded && (
                        <div className="mt-2 text-sm text-gray-600">
                          <span className="font-mono text-red-600">{conflict.currentValue}</span>
                          {' → '}
                          <span className="font-mono text-green-600">{conflict.newValue}</span>
                        </div>
                      )}
                    </div>
                      ...
                      <div className="px-4 pb-4 space-y-3 bg-white">
                          ...
                          <div className="text-sm">
                            <div className="flex justify-between items-center mb-2">
                              ...
                              <span className="text-xs text-gray-500">
                                {conflict.metadata.source} • {conflict.metadata.timestamp.toLocaleTimeString()}
                              </span>
                            </div>
                            <div className="font-mono text-sm">
                              <div className="text-red-600">- {conflict.currentValue || '(empty)'}</div>
                              <div className="text-green-600">+ {conflict.newValue}</div>
                            </div>
                          </div>
                        </div>
                        ...
                        {conflict.metadata.originalContext && (
                          <div className="bg-blue-50 rounded p-3 text-sm">
                            ...
                            <div className="text-blue-700 italic">"{conflict.metadata.originalContext}"</div>
                          </div>
                        )}

                        {/* Affected Fields */}
                        {conflict.metadata.affectedFields && conflict.metadata.affectedFields.length > 0 && (
                          <div className="text-sm">
                            <div className="font-medium mb-1">May Affect:</div>
                            <div className="flex flex-wrap gap-1">
                              {conflict.metadata.affectedFields.map(field => (
                                <span key={field} className="px-2 py-1 bg-yellow-100 text-yellow-800 rounded text-xs">
                                  {field}
                                </span>
                              ))}
                            </div>
                        ...
                        {conflict.suggestions && conflict.suggestions.length > 0 && (
                          <div>
                            <div className="font-medium text-gray-800 mb-2">Suggestions:</div>
                            <div className="space-y-2">
                              {conflict.suggestions.map(suggestion => (
                                <div key={suggestion.id} className="border rounded p-3 hover:bg-gray-50">
                                  ...
                                  {suggestion.consequences && (
                                    <div className="text-xs text-gray-500 mb-2">
                                      Consequences: {suggestion.consequences.join(', ')}
                                    </div>
                                  )}
                                  ...
                                  {suggestion.action === 'modify' && (
                                    <div className="mb-2">
                                      <input
                                        type="text"
                                        placeholder={suggestion.newValue || conflict.newValue}
                                        value={customValue}
