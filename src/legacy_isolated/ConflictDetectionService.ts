...
export interface FieldConflict {
  ...
  type: 'value_change' | 'constraint_violation' | 'incompatible_specs' | 'overwrite_warning';
  reason: string;
  confidence: number;
...
export class ConflictDetectionService {
  ...
  initialize(compatibilityLayer?: CompatibilityLayer): void {
    ...
    console.log('[ConflictDetection] Initialized with UC1 validation and compatibility layer');
  }
  ...
  async detectConflicts(
    ...
    const currentValue = this.getNestedValue(currentRequirements, section, fieldName);
    ...
    if (currentValue && currentValue !== 'Not Required' && newValue !== currentValue) {
      const valueChangeConflict = this.createValueChangeConflict(
        field, section, fieldName, currentValue, newValue, source, context
      );
    ...
    if (context?.uc1Spec) {
      const constraintConflicts = await this.detectConstraintViolations(
        field, newValue, context.uc1Spec, currentRequirements
      );
    ...
    const compatibilityConflicts = await this.detectCompatibilityIssues(
      field, newValue, currentRequirements
    );
    conflicts.push(...compatibilityConflicts);

    // 4. Semantic Confidence Warnings
    if (source === 'semantic' && context?.confidence && context.confidence < 0.7) {
      const confidenceWarning = this.createConfidenceWarning(
        field, section, fieldName, currentValue, newValue, context.confidence, context.originalRequest
      );
  ...
  private createValueChangeConflict(
    ...
    return {
      ...
      suggestions: this.generateValueChangeSuggestions(currentValue, newValue, source),
      metadata: {
        timestamp: new Date(),
        source,
  ...
  private createConfidenceWarning(
    ...
    return {
      ...
      reason: `Low confidence extraction (${Math.round(confidence * 100)}%). Please verify this interpretation.`,
      confidence,
      suggestions: [
        {
          id: 'verify',
  ...
  private async detectConstraintViolations(
    ...
    try {
      ...
      const validation = this.uc1Engine.validateSpecification(uc1Spec, newValue);
      if (!validation.isValid) {
        conflicts.push({
          id: `constraint_${field}_${Date.now()}`,
          field,
          section: field.split('.')[0],
          currentValue: this.getNestedValue(currentRequirements, ...field.split('.')),
          newValue,
          severity: 'error',
          type: 'constraint_violation',
          reason: `Value "${newValue}" violates UC1 constraints for ${uc1Spec.name}: ${validation.errors.join(', ')}`,
          confidence: 0.9,
  ...
  private async detectCompatibilityIssues(
    ...
    if (fieldName === 'processor_type' && newValue.toLowerCase().includes('intel')) {
      const memoryType = this.getNestedValue(currentRequirements, 'compute_performance', 'memory_type');
      if (memoryType && memoryType.toLowerCase().includes('ecc') && !newValue.toLowerCase().includes('xeon')) {
        conflicts.push({
          id: `compat_processor_memory_${Date.now()}`,
          field,
          section,
          currentValue: this.getNestedValue(currentRequirements, section, fieldName),
          newValue,
          severity: 'warning',
          type: 'incompatible_specs',
          reason: 'Intel consumer processors may not support ECC memory. Consider Xeon series for ECC support.',
          confidence: 0.8,
          suggestions: [
            {
              id: 'suggest_xeon',
              ...
              description: 'Change to Intel Xeon processor for ECC memory support',
    ...
    if (fieldName === 'max_power_consumption') {
      const processorType = this.getNestedValue(currentRequirements, 'compute_performance', 'processor_type');
      if (processorType && processorType.toLowerCase().includes('i7') && newValue.includes('<') && parseInt(newValue) < 15) {
        conflicts.push({
          ...
          severity: 'warning',
          type: 'incompatible_specs',
          reason: 'High-performance processors typically require more than 15W. Low power target may limit performance.',
          confidence: 0.85,
          suggestions: [
            {
              id: 'increase_power',
              ...
              description: 'Allow higher power consumption for better performance',
              newValue: '< 25W',
              confidence: 0.9,
            ...
            },
            {
              ...
              description: 'Keep low power target, may need lower-performance processor',
              confidence: 0.7,
  ...
  async resolveConflict(
    ...
    const resolution: ConflictResolution = {
      conflictId,
      action,
      newValue: newValue || (action === 'accept' ? conflict.newValue : conflict.currentValue),
      timestamp: new Date(),
      applied: false
    };
    ...
    try {
      ...
      console.log(`[ConflictDetection] Resolved conflict ${conflictId} with action: ${action}`);
      return resolution;

    } catch (error) {
      console.error(`[ConflictDetection] Failed to resolve conflict ${conflictId}:`, error);
      throw error;
    }
  }
  ...
  private generateConstraintSuggestions(uc1Spec: any, invalidValue: string): ConflictSuggestion[] {
    ...
    if (uc1Spec.constraints?.options) {
      suggestions.push({
        ...
        description: `Select from: ${uc1Spec.constraints.options.slice(0, 3).join(', ')}`,
        confidence: 0.8,
...
export const createConflictDetectionService = (uc1Engine: UC1ValidationEngine): ConflictDetectionService => {
