...
export class ArtifactManager {
  ...
  async addSpecificationToMapped(
    ...
    // SPRINT 3 FIX: Use UCDataLayer for hierarchy instead of UCValidationEngine
    ...
    console.log("[addSpecificationToMapped] found parentReqs", JSON.parse(JSON.stringify(parentReqs)));
    ...
    if (source === "conflict_resolution") {
      console.log(
        `[ArtifactManager] ⚠️  Skipping conflict detection for ${spec.id} (source: conflict_resolution)`
      );
    ...
    if (
      !originalRequest ||
      !originalRequest.includes("Auto-added as dependency")
    ) {
  ...
  private async autoFulfillDependencies(
    ...
    if (depth > 5 || visited.has(requirementId)) {
      console.warn(
        `[ArtifactManager] Skipping dependency fulfillment for ${requirementId} (depth=${depth}, visited=${visited.has(
          requirementId
        )})`
      );
      return;
    ...
    console.log(
      `[ArtifactManager] Auto-fulfilling ${requirement.dependencies.length} dependencies for ${requirementId}`
    );
    ...
    for (const dep of requirement.dependencies) {
      ...
      if (isDependencyActive) {
        console.log(
          `[ArtifactManager] ✓ Dependency ${depReqId} already satisfied`
        );
      ...
      console.log(
        `[ArtifactManager] ➕ Auto-adding dependency: ${depReqId} (${dep.rationale})`
      );
  ...
  private isRequirementActive(requirementId: string): boolean {
    for (const domain of Object.values(this.state.mapped.domains)) {
      if (domain.requirements[requirementId]) {
        const specCount = Object.keys(
          domain.requirements[requirementId].specifications
        ).length;
        if (specCount > 0) {
          return true;
        }
  ...
  private async addDependencyRequirement(requirementId: string): Promise<void> {
    ...
    if (!requirement) {
      console.warn(
        `[ArtifactManager] Cannot find requirement ${requirementId} for dependency fulfillment`
      );
      return;
    }

    // Get all specifications under this requirement
    const specIds = requirement.specification_ids;

    console.log(
      `[ArtifactManager] Adding ${specIds.length} default specifications for dependency ${requirementId}`
    );

    // Add each specification with its default value
    for (const specId of specIds) {
      const ucSpec = ucDataLayer.getSpecification(specId);

      console.log(
        `[ArtifactManager]   Checking ${specId}: spec=${!!uc1Spec}, default=${
      ...
      if (uc1Spec && uc1Spec.default_value) {
        ...
        if (uc1Spec.default_value !== "Not Required") {
          try {
            console.log(
              `[ArtifactManager]   → Adding ${specId} with value: ${uc1Spec.default_value}`
            );
            ...
            console.log(
              `[ArtifactManager]   ✓ Added ${specId} = ${uc1Spec.default_value}`
            );
          } catch (error) {
            console.error(
              `[ArtifactManager]   ✗ Failed to add ${specId}:`,
              error
            );
          }
        } else {
          console.log(
            `[ArtifactManager]   ⊘ Skipped ${specId} (default = "Not Required")`
          );
        }
      } else {
        console.log(
          `[ArtifactManager]   ⊘ Skipped ${specId} (no spec or no default_value)`
        );
      }
    }
  }
  ...
  private restoreSpecificationToMapped(specId: string, specBackup: any): void {
    ...
    if (parentReqs.length === 0) {
      console.error(
        `[Rollback] Cannot restore ${specId} - no parent requirements found`
      );
      return;
    }
    ...
    if (parentScenarios.length === 0) {
      console.error(
        `[Rollback] Cannot restore ${specId} - no parent scenario found`
      );
      return;
    }
    ...
    // SPRINT 3 FIX: Get scenario from UCDataLayer instead of ucEngine
    ...
    if (!this.state.mapped.domains[domain].requirements[requirement]) {
      ...
      this.state.mapped.domains[domain].requirements[requirement] = {
        id: requirement,
        name: uc8Requirement.name,
        ucSource: uc8Requirement,
        specifications: {},
      };
    }

    this.state.mapped.domains[domain].requirements[requirement].specifications[
      specId
    ] = specBackup;
  ...
  async moveNonConflictingToRespec(): Promise<void> {
    ...
    if (nonConflictingSpecs.length > 0) {
      ...
      console.log(
        `[ArtifactManager] Moved ${nonConflictingSpecs.length} non-conflicting specs to respec`
      );
    }
  }
  ...
  private addSpecificationToRespec(spec: UCArtifactSpecification): void {
    ...
    if (parentReqs.length === 0) {
      console.warn(
        `[ArtifactManager] Cannot add ${spec.id} to respec - no parent requirements found`
      );
      return;
    }
    ...
    if (parentScenarios.length === 0) {
      console.warn(
        `[ArtifactManager] Cannot add ${spec.id} to respec - no parent scenario found`
      );
      return;
    }
    ...
    if (!this.state.respec.domains[domainId]) {
      ...
      if (mappedDomain) {
        this.state.respec.domains[domainId] = {
          ...
          ucSource: mappedDomain.ucSource,
          requirements: {},
        };
      }
    }
    ...
    if (!this.state.respec.domains[domainId].requirements[reqId]) {
      ...
      if (mappedRequirement) {
        this.state.respec.domains[domainId].requirements[reqId] = {
          ...
          ucSource: mappedRequirement.ucSource,
          specifications: {},
        };
      }
    }
    ...
    // Add specification
    this.state.respec.domains[domainId].requirements[reqId].specifications[
      spec.id
    ] = {
      ...spec,
      timestamp: new Date(),
    ...
    for (const domain of Object.values(this.state.mapped.domains)) {
      for (const requirement of Object.values(domain.requirements)) {
        if (requirement.specifications[specId]) {
          ...
          const pendingIndex =
            this.state.mapped.metadata.pendingValidation.indexOf(specId);
          if (pendingIndex > -1) {
            this.state.mapped.metadata.pendingValidation.splice(
              pendingIndex,
              1
            );
          }
          ...
          console.log(
            `[ArtifactManager] Removed ${specId} from mapped artifact`
          );
          return;
        }
      }
  ...
  //             fieldData.value
  //           );
  //         console.log("todo zeev uc", requiredSpecifications);
  ...
  //         // console.log('TODO zeev uc getSpecIdFromField ' + specId, section, fieldName)
  //         // if (specId && fieldData.value && fieldData.value !== "") {
  //         //   // Get the real uc specification and pass it directly
  //         //   const ucSpec = this.ucEngine.getSpecification(specId);
  //         //   console.log('TODO zeev uc ucSpec for specId ' + specId, JSON.parse(JSON.stringify(ucSpec)))
  //         //   if (ucSpec) {
  //         //     // Pass the uc spec directly - let ArtifactManager handle the internal structure
  //         //     this.artifactManager
  //         //       .addSpecificationToMapped(ucSpec, fieldData.value)
  //         //       .catch((error) => {
  //         //         console.warn(
  ...
  //         //   } else {
  //         //     console.warn(
  //         //       `[CompatibilityLayer] uc specification not found for ${specId} (${section}.${fieldName})`
  //         //     );
  //         //   }
  //         // }
  //       });
