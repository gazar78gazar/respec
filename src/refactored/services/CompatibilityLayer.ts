...
import { UC1ArtifactSpecification } from "./ArtifactTypes";
...
export class CompatibilityLayer {
  ...
  private buildFieldMappings(): void {
    ...
    const commonMappings = [
      ...
      {
        section: "compute_performance",
        field: "response_latency",
        specId: "spc005",
      },
      {
        section: "compute_performance",
        field: "ai_gpu_acceleration",
        specId: "spc012",
      },
      {
        section: "compute_performance",
      ...
      {
        section: "io_connectivity",
        field: "fieldbus_protocol_support",
        specId: "spc033",
      },
      {
        section: "io_connectivity",
      ...
      {
        section: "io_connectivity",
        field: "serial_ports_amount",
        specId: "spc052",
      },
      {
        section: "io_connectivity",
        field: "serial_protocol_support",
        specId: "spc053",
      },
      {
        section: "io_connectivity",
        field: "wireless_extension",
        specId: "spc037",
      },
    ...
    console.log(
      `[CompatibilityLayer] Built ${this.fieldMappings.size} field mappings`
    );
  }
  ...
  syncArtifactToRequirements(requirements: LegacyRequirements): SyncResult {
    ...
    try {
      ...
      Object.values(respecArtifact.domains).forEach((domain) => {
        Object.values(domain.requirements).forEach((requirement) => {
          Object.values(requirement.specifications).forEach((spec) => {
            ...
            if (mapping) {
              const updated = this.updateRequirementField(
                requirements,
                mapping,
                spec
              );
      ...
      console.log(
        `[CompatibilityLayer] Synced ${result.updated.length} fields from artifact to requirements`
      );
    } catch (error) {
  ...
  validateSync(requirements: LegacyRequirements): {
    ...
    this.fieldMappings.forEach((mapping, specId) => {
      ...
      const requirementValue =
        requirements[mapping.section]?.[mapping.field]?.value;
  ...
  debugDumpMappings(): {
    ...
    return {
      fieldMappings: Array.from(this.fieldMappings.entries()).map(
        ([specId, mapping]) => ({
          specId,
      ...
      reverseMappings: Array.from(this.reverseFieldMappings.entries()).map(
        ([fieldKey, specId]) => ({
          fieldKey,
          specId,
        })
