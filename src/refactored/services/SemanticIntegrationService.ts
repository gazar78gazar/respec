...
export class SemanticIntegrationService {
  ...
  async processExtractedRequirements(
    ...
    try {
      ...
      console.log('[SemanticIntegration] ✅ Received', matchResults.length, 'UC1 matches');
  ...
  private _convertMatchesToFormUpdates(matches: MatchResult[]): EnhancedFormUpdate[] {
    ...
    for (const match of matches) {
      ...
      if (match.uc1Match.type !== 'specification') {
        console.log(`[SemanticIntegration] ⏭️  Skipping ${match.uc1Match.type} (not a specification)`);
        continue;
      }
  ...
  private async routeMatchesByType(matches: MatchResult[]): Promise<void> {
    for (const match of matches) {
      ...
      switch (uc1Match.type) {
        ...
        default:
          console.warn(`[Route] ❓ Unknown node type: ${uc1Match.type}`);
      }
    }
  }
  ...
  private async handleSpecificationMatch(
    ...
    if (this.artifactManager) {
      ...
      console.log(`[Route] ⚠️  No artifact manager - using legacy path`);
    }
  }
  ...
  private async handleDomainMatch(domainId: string): Promise<void> {
    ...
    if (this.artifactManager) {
      try {
        ...
        if (!domain) {
          console.warn(`[Route] ⚠️  Domain ${domainId} not found in UC1 schema`);
          return;
        }
